# Tomato 2D OBB RIoU fine-tuning from the best GWD checkpoint.
# This explicit config mirrors the successful GWD runtime because the GWD
# lazy-import config cannot participate in a normal inheritance chain.
#
# Run:
...
model = dict(
    ...
    bbox_head=dict(
        ...
        # Linear rotated-IoU keeps the true overlap objective while avoiding
        # the steep -log(IoU) gradient that became non-finite after epoch 1
        # when warm-starting from the GWD detector.
        loss_iou=dict(type=RotatedIoULoss, loss_weight=2.0, mode='linear')),
    train_cfg=dict(
        assigner=dict(
...
optim_wrapper = dict(
    ...
    optimizer=dict(
        ...
        # The GWD run started from random detector weights at 0.005.  This
        # RIoU run starts from converged epoch-50 weights, so use a 50x smaller
        # warm-start step to avoid immediately erasing the detector.
        muon_lr=0.0001,
...
# This checkpoint contains state_dict only, therefore optimizer state is
# intentionally fresh and no implicit resume is possible.
load_from = (
    'work_dirs/rddetr_tomato_gwd_amp_eval_full/'
    'best_rbbox_mAP_50_epoch_50.pth')
resume = False
