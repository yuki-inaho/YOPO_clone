...
class GDLoss(nn.Module):
    ...
    def forward(self,
        ...
        # ``torch.det`` in Gaussian distance is backed by LU factorization,
        # which has no CUDA Half kernel. Keep this small 2x2 geometry path in
        # fp32 while the detector forward remains in AMP. The cast preserves
        # the gradient path back to the fp16 prediction tensor.
