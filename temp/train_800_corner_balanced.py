...
deliberately an auxiliary.  The first attempt used weight 5.0 on an estimate
taken from a 17-degree pure-rotation error, but the measured term on real data
runs near 1.2 per sample -- the p90 angle error is 67 degrees and centre and
extent contribute too -- so 5.0 made the corner term thirty times the KLD and
the primary objective rather than an auxiliary.  Weight 0.3 puts the two on a
comparable scale.  The two runs bracket the range.
"""
